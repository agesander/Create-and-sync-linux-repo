...
# For other OS, please add necessary values in listed variables
...
function rsync_repo() {
  ...
  sleep 2
  # Создание папки в локальном репозитарии, если она не существует
...
if false; then
DATA_PATH=("centos/8-stream/AppStream/x86_64/os")
LOCAL_PATH=("/local_repo.online")
rsync -iavrt --delete  --exclude={'repo*'} "$MIRROR/$DATA_PATH/Packages" "$LOCAL_PATH/$DATA_PATH"

DATA_PATH=("centos/8-stream/BaseOS/x86_64/os")
LOCAL_PATH=("/local_repo.online")
rsync -iavrt --delete  --exclude={'repo*'} "$MIRROR/$DATA_PATH/Packages" "$LOCAL_PATH/$DATA_PATH"
fi #false
